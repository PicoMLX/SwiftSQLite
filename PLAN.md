...
8. **Minor follow-ups** — (a) `.indexes` still queries only the main schema, so indexes on TEMP tables aren't listed (union `sqlite_temp_schema`, as `.tables`/`.schema` now do); (b) a near-`UInt64.max` `statementTimeout` wraps the deadline through `&+` into the past (use a saturating add / treat the clamped max as "no deadline").
